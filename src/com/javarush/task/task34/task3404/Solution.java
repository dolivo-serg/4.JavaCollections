...
implementation by JavaRush team
*/
...
public class Solution {
    ...
    private double makeOperation(String s, double first, double second) {
        if ("+".equals(s)) return first + second;
        if ("=%".contains(s)) return 0.0 - second;
        if ("-@".contains(s)) return first - second;
        if ("*".equals(s)) return first * second;
        if ("/".equals(s)) return first / second;
        if ("^".equals(s)) return Math.pow(first, second);
        return -1;
    }

    private double makeFunction(String s, double first) {
        if ("s".equals(s)) return Math.sin(Math.toRadians(first));
        if ("c".equals(s)) return Math.cos(Math.toRadians(first));
        if ("t".equals(s)) return Math.tan(Math.toRadians(first));
        return -1;
    }
    ...
    private int getPriority(String s) {
        ...
        if ("^".contains(s)) return 3;
        if ("cst".contains(s)) return 4;
        return -1;
    }
